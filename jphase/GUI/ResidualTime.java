...
public class ResidualTime extends JDialog implements  ItemListener, ActionListener{
	public final String ACEPT = "Times";	
	...
	MainFrame window;
	public ResidualTime(MainFrame window){
		this.window = window;
		setTitle("JPhase - Residual Time Generator");
		...
		JLabel label2 = new JLabel("In order to get ResTime Values");
		...
		String message = arg0.getActionCommand();
	        ...
	        //addObject(category1, var);
		 	if(message.equals(ACEPT)){
		 		...
		 		this.p= Double.valueOf(f.getText()).doubleValue();
;
