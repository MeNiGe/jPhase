 ...
 * @version 1.0
 */
public class EMCorrHyperErlangFit {
     ...
     */
    public EMCorrHyperErlangFit(double[][] data){
    	this.data = data;
    	this.r = data.length;
    ...
    public double doFitNMR(HyperErlangVar var){
        ...
        if(m == 1){
            ...
            while( (Math.abs(logLHnow-logLHprev)>precision || iter ==0) && iter <= maxIter ){
                iter++;
                //compute Pm
                double[][] pm = new double[m][this.numSamples];
                for(int i = 0; i<m; i++){
                    for(int j = 0; j<this.numSamples; j++){
                        double temp = this.r*var.getR()[i]*Math.log(var.getLambdas()[i]) -   //r r_i ln(lambda_i) 
                        this.r*Utils.lnFactorial(var.getR()[i]-1) - 							// - r ln((n_i-1)!)
                        var.getLambdas()[i]*sumData[j] +  								// - lambda_i( sum_j x_j )
                        (var.getR()[i]-1)*Math.log(prodData[j]);						// + (r_i-1)log( prod_j x_j)
                ...
                double[] sumaPm = new double[this.numSamples];
    ...
    private void mStep(double[][] Qm, int[] r, double[] alphas, double[] lambdas, HyperErlangVar var){
        ...
        for(int i = 0; i<m; i++){
            if(sumaQm[i]>precision)lambdas[i]= this.r*r[i]*lambdas[i]/sumaQm[i];
        }
